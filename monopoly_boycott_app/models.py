...
class C(BaseConstants):
    ...
    PLAYERS_PER_GROUP = 3  # 1 monopolist + 5 consumers
    NUM_ROUNDS = 20
...
class Subsession(BaseSubsession):
    def creating_session(self):
        ...
        # Assign consumer endowments based on phase
        for p in players:
            if p.player_role == 'consumer':
                if self.round_number <= 5 or self.round_number >= 16:
                    p.endowment = random.choice([105, 110, 115, 120, 125])
                else:
                    p.endowment = random.choice([138, 140, 142, 144, 146])
...
class Player(BasePlayer):
    ...
    # Only for monopolist
    price = models.CurrencyField(min=100, max=160, blank=True)
